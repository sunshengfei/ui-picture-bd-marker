...
var ResizeAnnotation = function ResizeAnnotation(parentNode, boundRect) {
  ...
  this.setConfigOptions = function (newOptions) {
    _this2.options = _extends({}, _this2.options, newOptions.options);
    _this2.callback = _extends({}, _this2.callback, newOptions);
  };
  ...
  this.removeAnnotationEvent = function (e) {
    if (!_this2.options.editable) return;
    if (_this2.currentMovement) {
      var node = _this2.currentMovement.moveNode;
      var uuid = node.dataset.uuid;
      var tag = node.querySelector('.' + imageOpTag).dataset.id;
  ...
  this.drawAnnotation = function (rect) {
    ...
    for (var prop in resizeDotClasses) {
      ...
      if (i === 8) {
        resizeDot.className = (_this2.options.blurOtherDotsShowTags ? '' : '' + cls[i]) + ' ' + resizeDotClasses[prop];
        var opContent = document.createElement('div');
        opContent.className = 'g-image-op-content';
        if (_this2.options.tagLocationAlwaysTop) {
          ...
          if (_this2.options.tagLocation == defaultPositions.out_bottom) {
        ...
        if (_this2.options.trashPositionStart) {
        ...
        resizeDot.className = resizeDotClasses[prop] + ' ' + cls[i] + ' ' + (_this2.options.editable ? '' : 'hidden');
      }
    ...
    _this2.currentMovement = new Movement(annotation, 0, _this2.boundRect, _this2.options);
  ...
  this.removeSelectedAnnotation = function () {
    if (_this2.currentMovement) {
      ...
      cs.remove('selected');
      if (_this2.options.blurOtherDots) {
  ...
  this.selectMarkerByTagId = function (tagId) {
    ...
    if (tag) {
      ...
      _this2.removeSelectedAnnotation();
      _this2.currentMovement = new Movement(markerAnnotation, -1, _this2.boundRect, _this2.options);
  ...
  this.targetEventType = function (e) {
    ...
    if (el.classList.contains(cls[0])) {
      _this2.currentMovement = new Movement(parentEl, 0, _this2.boundRect, _this2.options);
    } else if (el.classList.contains(cls[1])) {
      _this2.currentMovement = new Movement(parentEl, 1, _this2.boundRect, _this2.options);
    } else if (el.classList.contains(cls[2])) {
      _this2.currentMovement = new Movement(parentEl, 2, _this2.boundRect, _this2.options);
    } else if (el.classList.contains(cls[3])) {
      _this2.currentMovement = new Movement(parentEl, 3, _this2.boundRect, _this2.options);
    } else if (el.classList.contains(cls[4])) {
      _this2.currentMovement = new Movement(parentEl, 4, _this2.boundRect, _this2.options);
    } else if (el.classList.contains(cls[5])) {
      _this2.currentMovement = new Movement(parentEl, 5, _this2.boundRect, _this2.options);
    } else if (el.classList.contains(cls[6])) {
      _this2.currentMovement = new Movement(parentEl, 6, _this2.boundRect, _this2.options);
    } else if (el.classList.contains(cls[7])) {
      _this2.currentMovement = new Movement(parentEl, 7, _this2.boundRect, _this2.options);
  ...
  this.options = _extends({}, defaultConfig.options, callback.options);
  this.annotationContainer = parentNode;
  this.boundRect = boundRect;
  this.actionDown = false;
  this.currentMovement = null;
  this.callback = _extends({}, defaultConfig, callback);
  this.data = [];
};
...
var BdAIMarker = function BdAIMarker(layer, draft, resizeAnnotation) {
  ...
  this.setConfigOptions = function (newOptions) {
    _this3.options = _extends({}, _this3.options, newOptions.options);
    if (_this3.resizeAnnotation) {
      _this3.resizeAnnotation.setConfigOptions(newOptions);
    }
  };

  this.mouseEventHandler = function (e, clientX, clientY) {
    var boundRect = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : _this3.boundRect();

    var eventType = e.type;
    _this3.moveX = clientX - boundRect.x;
    _this3.moveY = clientY - boundRect.y;
    if (eventTargetOnTransform) {
      _this3.resizeAnnotation.dragEventOn(e);
      return;
    }
    if (eventType === MOUSE_EVENT[0]) {
      ...
      if (_this3.actionDown) {
        _this3.dragTo(_this3.moveX, _this3.moveY);
        return;
      }
      _this3.actionDown = true;
      _this3.anchorX = _this3.moveX;
      _this3.anchorY = _this3.moveY;
      _this3.resetDraft();
      _this3.anchorAt(_this3.anchorX, _this3.anchorY);
    } else if (eventType === MOUSE_EVENT[1]) {
      if (_this3.actionDown) {
        _this3.dragTo(_this3.moveX, _this3.moveY);
      }
    } else if (eventType === MOUSE_EVENT[4]) {
      if (_this3.actionDown && _this3.resizeAnnotation) {
        _this3.resizeAnnotation.drawAnnotation(_this3.draftRect);
        _this3.resetDraft();
      }
      _this3.actionDown = false;
    } else {
      if (_this3.actionDown && _this3.filterOutOfBounds(_this3.moveX, _this3.moveY)) {
        if (_this3.resizeAnnotation) {
          _this3.resizeAnnotation.drawAnnotation(_this3.draftRect);
          _this3.resetDraft();
        }
  ...
  this.anchorAt = function (x, y) {
    if (!_this3.options.editable) return;
    _this3.draft.style.display = '';
    if (_this3.moveX < x) {
      _this3.draft.style.right = 100 * Math.abs(_this3.boundRect().width - x) / _this3.boundRect().width + '%';
      _this3.draft.style.left = '';
      _this3.draftRect = Object.assign(_this3.draftRect, {
        x: (100 * Math.abs(_this3.moveX) / _this3.boundRect().width).toFixed(3) + '%'
      });
    } else {
      _this3.draft.style.left = (100 * Math.abs(x) / _this3.boundRect().width).toFixed(3) + '%';
      _this3.draft.style.right = '';
      _this3.draftRect = Object.assign(_this3.draftRect, {
        x: (100 * Math.abs(x) / _this3.boundRect().width).toFixed(3) + '%'
      });
    }
    if (_this3.moveY < y) {
      _this3.draft.style.bottom = (100 * Math.abs(_this3.boundRect().height - y) / _this3.boundRect().height).toFixed(3) + '%';
      _this3.draft.style.top = '';
      _this3.draftRect = Object.assign(_this3.draftRect, {
        y: (100 * Math.abs(_this3.moveY) / _this3.boundRect().height).toFixed(3) + '%'
      });
    } else {
      _this3.draft.style.top = (100 * Math.abs(y) / _this3.boundRect().height).toFixed(3) + '%';
      _this3.draft.style.bottom = '';
      _this3.draftRect = Object.assign(_this3.draftRect, {
        y: (100 * Math.abs(y) / _this3.boundRect().height).toFixed(3) + '%'
      });
    }
  };

  this.filterOutOfBounds = function (x, y) {
    return x >= _this3.boundRect().x + _this3.boundRect().width + 2 || y >= _this3.boundRect().y + _this3.boundRect().height + 2 || x < 5 || y < 5;
  };

  this.resetDraft = function () {
    _this3.draftRect = { x: -1, y: -1, width: 0, height: 0 };
    _this3.draft.style.width = '0%';
    _this3.draft.style.height = '0%';
  };

  this.clearAll = function () {
    var annotations = _this3.layer.querySelectorAll('div.annotation');
    annotations.forEach(function (item) {
      item.remove();
    });
    _this3.renderData(void 0);
  };

  this.dragTo = function (x, y) {
    if (!_this3.options.editable) return;
    if (_this3.filterOutOfBounds(x, y)) {
      _this3.actionDown = false;
    }
    _this3.anchorAt(_this3.anchorX, _this3.anchorY);
    var widthRatio = (100 * Math.abs(x - _this3.anchorX) / _this3.boundRect().width).toFixed(3);
    var heightRatio = (100 * Math.abs(y - _this3.anchorY) / _this3.boundRect().height).toFixed(3);
    _this3.draftRect = Object.assign(_this3.draftRect, {
      width: widthRatio + '%',
      height: heightRatio + '%'
    });
    _this3.draft.style.width = _this3.draftRect.width;
    _this3.draft.style.height = _this3.draftRect.height;
  };
  ...
  this.renderData = function () {
    ...
    var ra = _this3.resizeAnnotation;
    if (ra) {
  ...
  this.setTag = function (tag) {
    if (_this3.resizeAnnotation && tag) {
      _this3.resizeAnnotation.setTagForCurrentMovement(tag);
    }
  };

  this.selectMarkerByTagId = function (tagId) {
    if (tagId) {
      _this3.resizeAnnotation.selectMarkerByTagId(tagId);
    }
  };

  this.dataSource = function () {
    if (_this3.resizeAnnotation) {
      return _this3.resizeAnnotation.dataSource();
    }
    return void 0;
  };

  this.dataForTag = function (tagId, uuid) {
    return _this3.resizeAnnotation.dataSourceOfTag(tagId, uuid);
  };

  if ((typeof configs === 'undefined' ? 'undefined' : _typeof(configs)) !== 'object') {
    throw 'Please provide a callback Config for BdAIMarker';
  }
  this.options = _extends({}, defaultConfig.options, configs.options);
  if (layer) {
    this.layer = layer;
    this.draft = draft;
    this.actionDown = false;
    ...
    this.resizeAnnotation = resizeAnnotation ? resizeAnnotation : new ResizeAnnotation(draft.parentNode, this.boundRect(), configs);
    var self = this;
