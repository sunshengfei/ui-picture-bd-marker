...
var ResizeAnnotation = function ResizeAnnotation(parentNode, boundRect) {
  ...
  this.dataSourceOfTag = function (tagId) {
  ...
  this.setTagForCurrentMovement = function (tagString) {
    var tag_str = '',
        tag_id = '';
    if (typeof tagString === 'string') {
      tag_id = tag_str = tagString;
    }
    if ((typeof tagString === "undefined" ? "undefined" : _typeof(tagString)) === 'object') {
    ...
    if (_this2.currentMovement) {
      ...
      var tag = node.querySelector("." + imageOpTag).dataset.tag;
      for (var i = 0; i < _this2.data.length; i++) {
        var value = _this2.data[i];
        if (value.tag === tag) {
          value.tag = tag_id;
  ...
  this.drawAnnotation = function (rect) {
    ...
    for (var prop in resizeDotClasses) {
      ...
      if (i === 8) {
        ...
        _tag.dataset.tag = tagId;
        _tag.className = "" + imageOpTag;
        _tag.innerText = tagString;
        _tag.setAttribute('data-id', tagId);
    ...
    var dts = _this2.dataTemplate(tagId, rect.x, rect.y, parseFloat(rect.x) + parseFloat(rect.width) + '%', parseFloat(rect.y) + parseFloat(rect.height) + '%');
    _this2.data.push(dts);
    _this2.callback.onUpdated(_this2.dataSource());
    _this2.callback.onDrawOne(dts);
  };
  ...
  this.selectAnnotation = function () {
    if (_this2.currentMovement) {
      ...
      var tag_id = node.querySelector("." + imageOpTag).getAttribute('data-id');
      _this2.callback.onSelect(_extends({
        name: tag_str
      }, _this2.dataSourceOfTag(tag_id)));
    }
  };
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.dataForTag = function (tagId) {
    return _this4.resizeAnnotation.dataSourceOfTag(tagId);
  };
};
